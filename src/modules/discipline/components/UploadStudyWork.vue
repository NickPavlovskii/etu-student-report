<template>
  ...
  >
    ...
    >
      ...
      <!-- Результаты проверки по шаблону -->
      <v-dialog
      ...
      >
        <v-card class="validation-result-card">
          ...
          <v-card-actions>
            <v-btn
              v-if="validationResult?.annotatedFileBase64 && validationResult?.annotatedFileName"
              variant="tonal"
              prepend-icon="mdi-file-eye-outline"
              @click="openAnnotatedFile"
            >
              {{ isAnnotatedPdf ? 'Просмотреть файл с замечаниями' : 'Скачать файл с замечаниями' }}
            </v-btn>
            <v-spacer />
            <v-btn
              variant="text"
...
<script setup lang="ts">
  ...
  const props = defineProps<{
    ...
    controls: any[];
    assessment: string;
    /** ID шаблона для проверки (если известен) */
    templateId?: string | number | null;
    /** Маппинг вид_контроля -> templateId (если templateId не задан) */
  ...
  const isAnnotatedPdf = computed(() =>
    (validationResult.value?.annotatedFileName ?? '').toLowerCase().endsWith('.pdf')
  );
  ...
  async function submitWork() {
    ...
    try {
      const tid =
        props.templateId ??
        (resolvedControlType.value && props.templateIdByWorkType?.[resolvedControlType.value]);
  ...
  function confirmUploadDespiteValidation() {
    ...
    if (p) {
      if (result?.percent != null && p.autoCheck) {
        (p as Record<string, unknown>).check = result.percent;
      }
      emit('submit', p);
      close();
    }
...
<style lang="scss">
/* Стили для диалога проверки (teleport) */
.validation-result-card { border-radius: 16px; padding: 8px 0; }
.validation-result-card .validation-header { display: flex; align-items: center; gap: 12px; padding: 16px 24px 8px; }
.validation-result-card .validation-title { margin: 0; font-size: 18px; font-weight: 600; color: #111827; }
.validation-result-card .validation-percent { font-size: 13px; color: #6b7280; margin-top: 4px; }
.validation-result-card .validation-errors { padding: 0 24px 16px; max-height: 280px; overflow-y: auto; }
.validation-result-card .validation-item { display: flex; align-items: flex-start; gap: 10px; padding: 8px 0; font-size: 14px; color: #374151; }
.validation-result-card .validation-item.error { color: #dc2626; }
.validation-result-card .validation-item.warning { color: #d97706; }
.validation-result-card .validation-criteria-list { padding: 0 24px 16px; max-height: 360px; overflow-y: auto; }
.validation-result-card .validation-item { display: flex; align-items: flex-start; gap: 10px; padding: 10px 0; font-size: 14px; border-bottom: 1px solid #f3f4f6; }
.validation-result-card .validation-item:last-child { border-bottom: none; }
.validation-result-card .validation-item.passed { color: #15803d; }
.validation-result-card .validation-item.error { color: #dc2626; }
.validation-result-card .validation-item.warning { color: #d97706; }
.validation-result-card .criteria-content { flex: 1; min-width: 0; }
.validation-result-card .criteria-title { font-weight: 600; display: block; margin-bottom: 2px; }
.validation-result-card .criteria-message { display: block; }
.validation-result-card .criteria-details { font-size: 12px; color: #6b7280; margin-top: 4px; }
</style>
