    ...
    <v-card class="add-template-modal">
      <div class="modal-header">
        <div class="modal-header-inner">
          <h2 class="modal-title">{{ isEdit ? 'Редактировать шаблон' : 'Новый шаблон проверки' }}</h2>
        </div>
        <v-btn icon variant="text" class="close-btn" @click="$emit('close')">
      ...
      <v-card-text class="modal-body">
        ...
        <div class="section">
          <div class="section-head">
            <v-icon size="20" class="section-icon">mdi-format-font</v-icon>
        ...
        <div class="section">
          <div class="section-head">
            <v-icon size="20" class="section-icon">mdi-file-document-outline</v-icon>
        ...
        <div class="section">
          <div class="section-head">
            <v-icon size="20" class="section-icon">mdi-image-multiple-outline</v-icon>
            <span>Иллюстрации и таблицы</span>
            <v-tooltip location="top">
              <template #activator="{ props }">
                <v-icon v-bind="props" size="16" class="ml-1">mdi-information-outline</v-icon>
              </template>
              Проверяются по триггерным словам (Рис., Рисунок, Таблица). При нарушении — предупреждение.
            </v-tooltip>
          </div>
          <div class="section-content">
            <div class="criteria-row">
        ...
        <div class="section">
          <div class="section-head">
            <v-icon size="20" class="section-icon">mdi-format-list-bulleted</v-icon>
            <span>Структура документа</span>
          </div>
          <div class="structure-grid">
            <v-checkbox v-model="form.hasTitlePage" label="Титульный лист" hide-details density="compact" color="primary" class="structure-check" />
            <v-checkbox v-model="form.hasToc" label="Оглавление" hide-details density="compact" color="primary" class="structure-check" />
            <v-checkbox v-model="form.hasIntroduction" label="Введение" hide-details density="compact" color="primary" class="structure-check" />
            <v-checkbox v-model="form.hasMainPart" label="Основная часть" hide-details density="compact" color="primary" class="structure-check" />
            <v-checkbox v-model="form.hasConclusion" label="Заключение" hide-details density="compact" color="primary" class="structure-check" />
            <v-checkbox v-model="form.hasBibliography" label="Список литературы" hide-details density="compact" color="primary" class="structure-check" />
            <v-checkbox v-model="form.hasAppendices" label="Приложения" hide-details density="compact" color="primary" class="structure-check" />
          </div>

          <!-- Загрузка макета титульного листа (когда включён титульный лист) -->
          <div v-if="form.hasTitlePage" class="title-page-block">
            <div class="title-page-inner">
              <div class="title-page-head">
                <v-icon size="22" class="title-page-icon">mdi-file-document-outline</v-icon>
              ...
              <p class="title-page-hint">
                Загрузите DOC, DOCX или PDF — система извлечёт обязательные элементы для проверки титула
              </p>
            </div>
          </div>
        </div>
      ...
      <v-card-actions class="modal-actions">
        <v-spacer />
        <v-btn variant="text" class="btn-cancel" @click="$emit('close')">
          Отмена
        </v-btn>
        <v-btn color="primary" class="btn-submit" @click="submit">
...
import type { AddTemplateForm } from '../types';
...
const fontSizeOptions = ['9 пт', '10 пт', '11 пт', '12 пт', '14 пт', '16 пт', '18 пт', '20 пт', '22 пт', '24 пт'];

const illNumberingOptions = ['Сквозная', 'По разделам', 'В рамках раздела'];

const figurePositionOptions = ['По центру страницы', 'Слева', 'Справа', 'По ширине страницы'];

const figureCaptionOptions = ['Под рисунком', 'Над рисунком', 'Под рисунком по центру'];
...
const tablePositionOptions = ['По центру страницы', 'Слева', 'Справа', 'По ширине страницы'];
...
async function onTitlePageFileSelected(event: Event) {
  ...
  try {
    ...
    fd.append('file', file);
    const res = await fetch(`${base}/api/templates/${props.templateId}/title-page-template`, {
      method: 'POST',
      body: fd,
    });
    if (!res.ok) {
