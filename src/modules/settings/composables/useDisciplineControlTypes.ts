 ...
 */
import type { ControlTypeItem } from '../modal';
...
export function mergeControlTypesWithSettings(
  ...
  const storedByTitle = new Map(
    stored.map((ct) => [ct.title.toLowerCase(), ct])
  );
...
export function saveControlTypesForDiscipline(
  disciplineId: string,
...
export function getVisibleControlTitles(
  disciplineId: string
): string[] | undefined {
  const config = loadFromStorage();
...
export function getDisplayedTopicsByControlType(
  disciplineId: string
  ...
  for (const ct of types) {
    ...
    if (key) {
      out[ct.title] =
        ct.displayedTopics !== undefined ? ct.displayedTopics : undefined;
    }
  }
  return out;
}
